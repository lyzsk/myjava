...
public class LongestCommonPrefix {
    ...
    /**
     * 先排序，后比较头尾 还不如多加个 i >= strs[j].length()判断...
     * 
     * @param strs
     * @return
     */
    public String longestCommonPrefixSolution2(String[] strs) {
        ...
        Arrays.stream(strs).sorted(Comparator.comparingInt(String::length));
    ...
    /**
     * 二分法 每次取查找范围的中间值 mid， 判断每个字符串的长度为 mid 的前缀是否相同, 如果相同则最长公共前缀的长度一定大于或等于 mid 如果不相同则最长公共前缀的长度一定小于 mid
     * 
     */
    ...
    public String longestCommonPrefixSolution4(String[] strs) {
        if (strs == null || strs.length == 0) {
            ...
            return longestCommonPrefixSolutionRecurse(strs, 0, strs.length - 1);
        }
    }

    public String longestCommonPrefixSolutionRecurse(String[] strs, int start, int end) {
        if (start == end) {
            return strs[start];
        } else {
            int mid = (end - start) / 2 + start;
            String lcpLeft = longestCommonPrefixSolutionRecurse(strs, start, mid);
            String lcpRight = longestCommonPrefixSolutionRecurse(strs, mid + 1, end);
